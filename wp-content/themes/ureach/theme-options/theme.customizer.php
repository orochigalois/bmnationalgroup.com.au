	...
	function ureach_customizer_css_template() {
		...
		if (is_array($schemes) && count($schemes) > 0) {
			foreach ($schemes as $scheme) {
				echo '<' . esc_html($tmpl_holder) . ' type="text/html" id="tmpl-ureach-color-scheme-'.esc_attr($scheme).'">'
...
if (!function_exists('ureach_save_override_options')) {
	//Handler of the add_action('save_post', 'ureach_save_override_options');
					...
					$output .= $img && !ureach_is_inherit($img)
							? '<span>'
									. (in_array(ureach_get_file_ext($img), array('gif', 'jpg', 'jpeg', 'png'))
											? '<img src="' . esc_url($img) . '" alt="' . esc_html__('Image', 'ureach') . '">'
		...
		case 'scheme_editor':
			...
			foreach ($value as $scheme=>$v) {
				...
				foreach ($groups as $group_name=>$group_data) {
					$output .= '<span class="ureach_scheme_editor_header_cell" title="'.esc_html($group_data['description']).'">' 
				...
				foreach ($colors as $color_name=>$color_data) {
					$output .= '<div class="ureach_scheme_editor_row">'
								. '<span class="ureach_scheme_editor_row_cell" title="'.esc_html($color_data['description']).'">'
